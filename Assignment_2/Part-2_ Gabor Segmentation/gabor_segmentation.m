...
for jj = 1 : length(gaborFilterBank)
    real_out =gaborFilterBank.filterPairs(:,:,1);  % \\TODO: filter the grayscale input with real part of the Gabor
    imag_out =gaborFilterBank.filterPairs(:,:,2);  % \\TODO: filter the grayscale input with imaginary part of the Gabor
...
for jj = 1:length(featureMaps)
    ...
    featureMags{jj} =real_part+imag_part; % \\TODO: Compute the magnitude here
...
tic
pixLabels =kmeans(features,2); % \\TODO: Return cluster labels per pixel
ctime = toc;
