...
for jj = 1 : length(gaborFilterBank)
    real_out =  gaborFilterBank(jj).filterPairs(:,:,1);%imfilter(img_gray,gaborFilterBank(jj).filterPairs(:,:,1));% \\TODO: filter the grayscale input with real part of the Gabor
    imag_out =  gaborFilterBank(jj).filterPairs(:,:,2);%imfilter(img_gray,gaborFilterBank(jj).filterPairs(:,:,2));% \\TODO: filter the grayscale input with imaginary part of the Gabor
    ...
    saveas(figure(2),['Gabor_affect/Gabor_affect_onDomain_' num2str(jj) '.png']);                                                                                                       
    pause(0.01)
    end
end
